  ...
  Future<void> _doCancel(int index) async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        _toast('Booking for ${booking.room} has been cancelled.', bg: const Color(0xFFEF4444));
      } else {
                    ...
                    : _bookings.isEmpty
                        ...
                        : ListView.builder(
                            padding: EdgeInsets.fromLTRB(
                              20, 0, 20, MediaQuery.of(context).padding.bottom + 60,
                            ),
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: const LinearGradient(colors: [Color(0xFFFF6B6B), Color(0xFFD61F26)]),
            ...
            Container(
              ...
              child: Column(
                children: [
                  _MyBookingsPageState._rowInfo(Icons.meeting_room_rounded, 'Room', room),
                  const SizedBox(height: 8),
                  _MyBookingsPageState._rowInfo(Icons.calendar_today_rounded, 'Date', date),
                  const SizedBox(height: 8),
                  _MyBookingsPageState._rowInfo(Icons.access_time_rounded, 'Time', time),
                ],
              ),
            ),
...
class _BookingCardState extends State<_BookingCard> {
  ...
  Color get _statusColor {
    switch (widget.booking.status) {
      ...
      case BookingStatus.pending:   return const Color(0xFF7C3AED);
      case BookingStatus.cancelled: return const Color(0xFFEF4444);
      case BookingStatus.rejected:  return const Color(0xFFF59E0B);
    }
  }

  Color get _statusBgColor {
    switch (widget.booking.status) {
      case BookingStatus.confirmed: return const Color(0xFFD1FAE5);
      case BookingStatus.pending:   return const Color(0xFFEDE9FE);
      case BookingStatus.cancelled: return const Color(0xFFFEE2E2);
      case BookingStatus.rejected:  return const Color(0xFFFEF3C7);
    }
  }
  ...
  IconData get _statusIcon {
    switch (widget.booking.status) {
      ...
      case BookingStatus.rejected:  return Icons.info_rounded;
    }
  }
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            _statusChip(_statusText, _statusBgColor, _statusColor, _statusIcon),
                          ],
                        ),
                        const SizedBox(height: 18),
                        _dateTimePanel(widget.booking.date, widget.booking.time),
                        const SizedBox(height: 16),
                        ...
                        if (widget.booking.status == BookingStatus.pending && widget.onRequestCancel != null)
                          _cancelButton(widget.onRequestCancel!),

                        if (widget.booking.status == BookingStatus.cancelled || widget.booking.status == BookingStatus.rejected)
                          _rejectedBox(widget.booking.status, widget.booking.rejectReason),
                      ],
                    ),
                  ),
                ],
  ...
  Widget _roomTitle(String room) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Text('ROOM', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: Color(0xFF8B6F47), letterSpacing: 1.2)),
      const SizedBox(height: 2),
      Text(room, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: Color(0xFF1A1A2E), letterSpacing: -0.5, height: 1.1)),
    ],
  );
  ...
  Widget _statusChip(String text, Color bg, Color fg, IconData icon) => Container(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      Text(text, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: fg, letterSpacing: 0.3)),
    ]),
  );
  ...
  Widget _dateTimePanel(String date, String time) => Container(
    ...
    child: Row(children: [
      ...
      const SizedBox(width: 12),
      Expanded(child: Text(date, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF1A1A2E)))),
      const SizedBox(width: 12),
      Container(
        padding: const EdgeInsets.all(8),
        decoration: BoxDecoration(
      ...
      const SizedBox(width: 12),
      Text(time, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF5D6CC4))),
    ]),
  );
  ...
  Widget _cancelButton(VoidCallback onTap) => SizedBox(
    ...
    child: ElevatedButton(
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.center, children: const [
        Icon(Icons.cancel_rounded, size: 18), SizedBox(width: 8),
        Text('Cancel Booking', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14)),
      ]),
    ),
  );
  ...
  Widget _rejectedBox(BookingStatus status, String? reason) {
    ...
    final base = isRejected ? const Color(0xFFF59E0B) : const Color(0xFFEF4444);
